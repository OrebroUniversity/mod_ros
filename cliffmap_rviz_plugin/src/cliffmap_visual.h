...
class CLiFFMapVisual {
 public:
  CLiFFMapVisual(Ogre::SceneManager* scene_manager,
                 Ogre::SceneNode* parent_node);
  virtual ~CLiFFMapVisual();

  // Configure the visual to show the data in the message.
  void setMessage(const cliffmap_ros::CLiFFMapMsg::ConstPtr& msg);

  // Set the pose of the coordinate frame the message refers to.
  // These could be done inside setMessage(), but that would require
  // calls to FrameManager and error handling inside setMessage(),
  // which doesn't seem as clean.  This way CLiFFMapVisual is only
